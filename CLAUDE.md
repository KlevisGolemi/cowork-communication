...
- `mcp/src/queueClient.ts` — HTTP client to webhook-queue (injects `WEBHOOK_SECRET` header, typed `QueueResult`)
- `mcp/src/adminApi.ts` — REST router proxying queue operations + exposes config/secrets
